...
# - Use Zero 2 W DTB (bcm2710-rpi-zero-2-w)
{
  ...
  # Kernel: use LZ4 compression (faster decompress) and ensure rootfs/MMC drivers built-in
  echo "BR2_LINUX_KERNEL_CONFIG_FRAGMENT_FILES=\"$BR_EXT_DIR/board/hvacx/rpi0_2w/kernel-lz4.fragment $BR_EXT_DIR/board/hvacx/rpi0_2w/kernel-rootfs.fragment\""
  # Ensure host lz4 tool present if needed by build
  echo "BR2_PACKAGE_HOST_LZ4=y"
  # Ensure upstream packs our cmdline/config into boot.vfat
  echo "BR2_PACKAGE_RPI_FIRMWARE_CMDLINE_FILE=\"$BR_EXT_DIR/board/hvacx/rpi0_2w/cmdline.txt\""
  echo "BR2_PACKAGE_RPI_FIRMWARE_CONFIG_FILE=\"$BR_EXT_DIR/board/hvacx/rpi0_2w/config.txt\""
